    ...
    public DefaultMcpToolRegistry()
    {
        var tools = new[]
        {
            ...
            new McpToolMetadata("list_task_locks", "libr4-agent-bridge", "List task locks",
                McpToolRiskLevel.Low, McpExecutionLaneKind.Internal, new[] { "coordination" }),
            new McpToolMetadata("browser.smoke", "obscura-browser-lane", "Obscura browser smoke (navigate + screenshot)",
                McpToolRiskLevel.High, McpExecutionLaneKind.Browser, new[] { "ui", "e2e", "smoke", "obscura" }),
            new McpToolMetadata("browser.auth", "obscura-browser-lane", "Obscura browser auth flow (PerformActions login + screenshot)",
                McpToolRiskLevel.High, McpExecutionLaneKind.Browser, new[] { "ui", "e2e", "auth", "obscura" }),
